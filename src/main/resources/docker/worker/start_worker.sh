...
sed -i "s/^DISPATCHERS=.*/DISPATCHERS=$SCHEDULER_DOMAIN/g" /xwhep/conf/xtremweb.worker.conf
...
sed -i "s/^SSLKEYSTORE=.*/SSLKEYSTORE=\/etc\/ssl\/certs\/java\/cacerts/g" /xwhep/conf/xtremweb.worker.conf
sed -i "s/^SSLKEYPASSWORD=.*/SSLKEYPASSWORD=changeit/g" /xwhep/conf/xtremweb.worker.conf

# add the TMPDIR variable if defined
if [ ! -z $TMPDIR ] ; then
	sed -i "s/^#TMPDIR=.*/TMPDIR=$TMPDIR/g" /xwhep/conf/xtremweb.worker.conf
fi

# Add defined SHAREDAPPS variable if defined, otherwise use a default value  
if [ ! -z $SHAREDAPPS ] ; then 
	sed -i "s/^#SHAREDAPPS=.*/SHAREDAPPS=$SHAREDAPPS/g" /xwhep/conf/xtremweb.worker.conf
else sed -i "s/^#SHAREDAPPS=.*/SHAREDAPPS=docker/g" /xwhep/conf/xtremweb.worker.conf
fi

# Add SHAREDPACKAGES only if it is given in parameters
if [ ! -z $SHAREDPACKAGES ] ; then
	sed -i "s/^#SHAREDPACKAGES=.*/SHAREDPACKAGES=$SHAREDPACKAGES/g" /xwhep/conf/xtremweb.worker.conf
fi

# Set default value for Login
if [ ! -z $LOGIN ] ; then
	sed -i "s/^LOGIN=.*/LOGIN=$LOGIN/g" /xwhep/conf/xtremweb.worker.conf
else sed -i "s/^LOGIN=.*/LOGIN=vworker/g" /xwhep/conf/xtremweb.worker.conf
fi

# Set default value for Password
if [ ! -z $PASSWORD ] ; then
	sed -i "s/^PASSWORD=.*/PASSWORD=$PASSWORD/g" /xwhep/conf/xtremweb.worker.conf
else sed -i "s/^PASSWORD=.*/PASSWORD=vworkerp/g" /xwhep/conf/xtremweb.worker.conf
fi

# Set the LoggerLevel
if [ ! -z $LOGGERLEVEL ] ; then 
	sed -i "s/^LOGGERLEVEL=.*/LOGGERLEVEL=$LOGGERLEVEL/g" /xwhep/conf/xtremweb.worker.conf
fi

cat /etc/hosts

# Start the worker 
/xwhep/bin/xtremweb.worker console
