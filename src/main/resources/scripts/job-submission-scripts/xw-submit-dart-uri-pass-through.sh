...
#-----------------------------------------------------------------------------
DART_INSTRUMENT=${1:-DARTAcousticG}
DART_WINDOW=${2:-Rectangle}

XW_APP_NAME=dart.jar
XW_APP_ARGS=(-infile "$DART_INSTRUMENT.wav" -outfile "$DART_INSTRUMENT-$DART_WINDOW.txt" -nofreqpoints 1 -noharmonics 1 -fft_window "$DART_WINDOW")

XW_INPUT_TYPE=' an explicit URI-PASS-THROUGH referencing 2 URLs'
XW_INPUTS=("${BASE_URI}$DART_INSTRUMENT.wav"  "${BASE_URI}pools-duplicate.txt")
