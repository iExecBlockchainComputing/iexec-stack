...
public final class DBCommandGetWorks extends DBCommand implements CommCallback {
	 ...
	 */
	public DBCommandGetWorks(final DBInterface dbi) throws IOException {
		super();
		dbInterface = dbi;
	}
	...
	public XMLable exec(final XMLRPCCommand command)
		...
		mileStone.println("<getworks status=\"" + status + "\" >");

		final Vector<UID> ret = (Vector<UID>) dbInterface.getAllJobs(command);
		XMLVector v = null;
		if (ret != null) {
			v = new XMLVector(ret);
		}
