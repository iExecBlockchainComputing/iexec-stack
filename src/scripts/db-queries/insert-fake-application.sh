...
OWNER_UID=`mysql $MYSQLOPTS -B -e "select uid,login from users where login=\"$USERLOGIN\"" | tail -1 | cut -f 1`
if [ $? -ne 0 ] ; then
	fatal "Can't retrieve \"$USERLOGIN\" user"
fi
[ -z $OWNER_UID ] && fatal "Can't retrieve 'admin' user"

APPTEST_UID=`mysql $MYSQLOPTS -B -e "select uid,name from apps where name=\"$APPNAME\" " | tail -1 | cut -f 1`
[ ! -z $APPTEST_UID ] && fatal "Application \"$APPNAME\" already exists"
