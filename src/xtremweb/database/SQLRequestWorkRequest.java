...
public class SQLRequestWorkRequest extends SQLRequest {
	...
	private static final String WORKREQUESTCRITERIAS = " AND maintable.status='%s'"
			...
			+ " AND ((maintable.DATADRIVENUID IS NULL) OR (maintable.DATADRIVENUID='') OR ((maintable.DATADRIVENUID=datas.UID) AND (datas.PACKAGE IS NOT NULL) AND (datas.PACKAGE IN (%s))))";
