...
public abstract class SQLRequestAccessible extends SQLRequest {
	 ...
	 */
	private static final String TABLENAMES = "%s as " + MAINTABLEALIAS + ","
			+ UserInterface.TABLENAME;
	private static final String TABLEUSERS = UserInterface.TABLENAME + " as "
			+ MAINTABLEALIAS + "," + UserInterface.TABLENAME;

	/**
	 * This contains criteria for the WHERE part of the SQL request.
	 */
	public static final String CRITERIAS = UserInterface.TABLENAME
			+ ".uid='%s'"
			+ " AND users.isdeleted='false'"
			+ " AND ("
			+ " users.uid=maintable.uid" // only for "users" table : a user can retrieve itself
			+ " OR users.uid=maintable.owneruid" // is the requester the owner?
			+ " OR users.rights='" + UserRightEnum.SUPER_USER + "'" // is the requester an administrator ?
			+ " OR (users.rights='" + UserRightEnum.VWORKER_USER + "'" // a VWORKER can access an object that has the sticky bit
			+ " AND maintable.accessrights & " + XWAccessRights.STICKYBIT_INT + " = " + XWAccessRights.STICKYBIT_INT + ")"
			+ " OR (maintable.accessrights & %d = %d)" // is the object accessible for all users
			+ " OR ("// finally, is the object defined in a group
			+ "     (maintable.accessrights & %d = %d)" 
			+ "     AND users.usergroupuid IN (" // if so, is the requester member
												// of the object group ?
			+ "        SELECT usergroups.uid" + " FROM users,usergroups"
			+ "        WHERE usergroups.uid=users.usergroupuid" + " AND users.uid=maintable.owneruid" 
			+ "      )" // end IN "SELECT..."
	 ...
	 * @see #CRITERIAS
	 */
	public static final String CRITERIAS_HSQL = UserInterface.TABLENAME + ".uid='%s'"
			+ " AND users.isdeleted='false'" + " AND ("
			+ " users.uid=maintable.uid" + " OR users.uid="
			+ "maintable.owneruid" + " OR users.rights='"
			+ UserRightEnum.SUPER_USER + "'" + " OR (users.rights='"
			+ UserRightEnum.VWORKER_USER + "'" + " AND bitand(" 
			+ "maintable.accessrights, " + XWAccessRights.STICKYBIT_INT + ") = "
			+ XWAccessRights.STICKYBIT_INT + ")" + " OR (bitand("
			+ "maintable.accessrights, %d) = %d)" + " OR ((bitand("
			+ "maintable.accessrights, %d) = %d)"
			+ " AND users.usergroupuid IN (" + " SELECT usergroups.uid"
			+ " FROM " + getDbName()
			+ ".users," + getDbName()
			+ ".usergroups" + " WHERE usergroups.uid=users.usergroupuid"
	 ...
	 *            is the table name
	 */
	public SQLRequestAccessible(String t, UserInterface u) throws IOException {
		this();
		setTableName(t);
	 ...
	 *            is the row UID
	 */
	public SQLRequestAccessible(String t, UserInterface u, UID uid) throws IOException {
		this(t, u);
		if (uid != null) {
	 ...
	 */
	public SQLRequestAccessible(String t, UserInterface u, String c) throws IOException {
		this(t, u);
		setMoreCriterias(c);
	}
	 ...
	 */
	public SQLRequestAccessible(String t, UserInterface u, ColumnSelection s)
			throws IOException {
		this(t, u);
	 ...
	 */
	public SQLRequestAccessible(String t, UserInterface u, ColumnSelection s, UID uid)
			throws IOException {
		this(t, u, uid);
		setColumnSelection(s);
	}
	 ...
	 */
	public SQLRequestAccessible(String t, UserInterface u, ColumnSelection s, String c)
			throws IOException {
		this(t, u, c);
		setColumnSelection(s);
	}
	...
	public String criterias() throws IOException {
		...
		final String ret = String.format(getCriterias(), getUser().getUID().toString(),
				getOtherAccess(), getOtherAccess(), getGroupAccess(), getGroupAccess());
	 ...
	 */
	public final void setOtherAccess(int otherAccess) {
	 ...
	 */
	public final void setGroupAccess(int groupAccess) {
