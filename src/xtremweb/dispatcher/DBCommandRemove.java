...
public final class DBCommandRemove extends DBCommand implements CommCallback {
	 ...
	 */
	public DBCommandRemove(final DBInterface dbi) throws IOException {
		super();
		dbInterface = dbi;
	}
	...
	public XMLable exec(final XMLRPCCommand command)
		...
		if (!dbInterface.remove(command)) {
			mileStone.println("<error method='remove' msg='object not found' />");
			mileStone.println("</remove>");
