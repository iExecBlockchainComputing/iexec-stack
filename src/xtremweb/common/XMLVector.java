...
public final class XMLVector extends XMLValue {
	 ...
	 *             on XML error
	 */
	public XMLVector(DataInputStream input) throws IOException, SAXException {
		this(new Vector());
		setEmpty(false);
	 ...
	 *             on XML error
	 */
	public XMLVector(String input) throws IOException, SAXException {
	 ...
	 *             on XML error
	 */
	public XMLVector(Attributes attrs) {
		this(new Vector());
		setEmpty(false);
		fromXml(attrs);
	}
	...
	public String toXml() {
		...
		if (value == null) {
			return "<" + getXMLTag() + " " + getColumnLabel(SIZE)
					+ "=\"0\" ></" + getXMLTag() + ">";
		}

		String ret = "<" + getXMLTag() + " " + getColumnLabel(SIZE) + "=\""
				+ ((Vector<XMLValue>) value).size() + "\" >";
	...
	public void toXml(DataOutputStream o) throws IOException {
		...
		final Object value = getValue();
		if (value == null) {
			ret = "<" + getXMLTag() + " " + getColumnLabel(SIZE) + "=\"0\" ></"
					+ getXMLTag() + ">";
		...
		ret = "<" + getXMLTag() + " " + getColumnLabel(SIZE) + "=\""
				+ ((Vector<XMLValue>) value).size() + "\" >";
		strb = ret.getBytes(XWTools.UTF8);
		o.write(strb);
		strb = null;
	...
	public void xmlElementStart(String uri, String tag, String qname,
		...
		final Object value = getValue();
		logger.finest("xmlElementStart (" + qname + ") " + "expectedSize = "
				+ expectedSize + "  values.size = "
				+ (value != null ? ((Vector<XMLValue>) value).size() : 0)
		...
		if (((value == null) || (((Vector<XMLValue>) value).size() == 0))
			...
			if (currentIndex >= ((Vector<XMLValue>) value).size()) {
				logger.finest("values[" + currentIndex + "]  = new XMValue("
						+ qname + ")");
				...
				logger.debug("values[" + currentIndex + "].xmlElementStart("
						+ qname + ")");
				((Vector<XMLValue>) value).elementAt(currentIndex)
	...
	public void xmlElementStop(String uri, String tag, String qname)
		...
		final Object value = getValue();
		if (currentIndex < ((Vector<XMLValue>) value).size()) {
			((Vector<XMLValue>) value).elementAt(currentIndex).xmlElementStop(
					uri, tag, qname);
		}
		...
		getLogger().finest(
				"xmlElementStop (" + qname + ") " + "expectedSize = "
						+ expectedSize + "  currentIndex = " + currentIndex
						+ "  nested = " + nested);
	...
	public static void main(String[] argv) {
		try {
			...
			if (argv.length == 1) {
				xmlv = new XMLVector(new DataInputStream(new FileInputStream(
						argv[0])));
			}
